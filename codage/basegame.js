...
function setupMainMenu() {
    ...
    }, 'Mode Entrainement', 'conversions', 5, 10, 25, 50, 100);

    constructButtonSet(function(num){
        clearContentDiv();
        createNewTimeGame(num);
    }, 'Mode Temps', 'secondes', 30, 60, 90, 120, 150, 300);
}
...
function constructGame() {
    ...
    jumbotron.append($('<p/>', {
        class: 'question',
        html: '<span class="qnum">12AB</span> <span class="qbase">octal</span> <br/> en <span class="qtarget">hexadecimal</span>'
    }));
...
function constructGameOver() {
    ...
    if (mode == 'time') {
        $('.content').append($('<p/>', {
            class: 'result',
            html: 'Vous avez fait <span class="squestions bold">0</span> conversion' + ((correctQuestions == 1) ? '.' : 's.')
        }));
        $('.content').append($('<p/>', {
            class: 'settings',
            html: 'Limite de temps: <span class="smaxtime"></span><br/>Conversions Réussites: <span class="squestions">A</span>/<span class="smaxquestions">B</span>'
        }));
    } else {
        $('.content').append($('<p/>', {
            class: 'result',
            html: 'Vous avez passé <span class="stime bold">XX:XX</span> pour faire les conversions.'
        }));
        $('.content').append($('<p/>', {
            class: 'settings',
            html: 'Nombre de Conversions: <span class="smaxquestions">B</span>'
        }));
    }
